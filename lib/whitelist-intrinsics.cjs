...
exports.default = whitelistIntrinsics;

var _whitelist = _interopRequireWildcard(require("./whitelist.cjs"));
...
function whitelistIntrinsics(intrinsics) {
  ...
  function isWhitelistPropertyValue(path, value, prop, permit) {
    if (typeof permit === 'object') {
      ...
      whitelistProperties(path, value, permit); // The property is whitelisted.
  ...
  function isWhitelistProperty(path, obj, prop, permit) {
    ...
    return isWhitelistPropertyValue(`${path}<get>`, desc.get, prop, permit.get) && isWhitelistPropertyValue(`${path}<set>`, desc.set, prop, permit.set);
  }
  /**
   * getSubPermit()
   */
  ...
  function getSubPermit(permit, prop) {
    ...
    if (permit['**proto**'] === 'FunctionPrototype') {
      if (hasOwnProperty(_whitelist.FunctionInstance, prop)) {
        return _whitelist.FunctionInstance[prop];
      }
    }
  ...
  whitelistProperties('intrinsics', intrinsics, _whitelist.default);
}